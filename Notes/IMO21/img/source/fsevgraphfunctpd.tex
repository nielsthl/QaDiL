...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm,
 ...
 % \tikzstyle{every state}=[fill=red,draw=none,text=white]
  ...
  \path (B) edge              node {$\pd{F}{a}$} (A);
  \path (C) edge              node {$\pd{F}{b}$} (A);
  \path (E) edge              node {$\pd{b}{d}$} (C);
  \path (F) edge              node {$\pd{b}{e}$} (C);
  \path (D) edge              node {$\pd{a}{c}$} (B);
  \path (G) edge              node {$\pd{c}{x}$} (D);
  \path (G) edge [bend right]  node {$\pd{d}{x}$} (E);
  \path (H) edge [bend left] node {$\pd{c}{y}$} (D);
  \path (H) edge              node {$\pd{e}{y}$} (F);
  \path (H) edge [bend left]  node {$\pd{F}{y}$} (A);
\end{tikzpicture}
